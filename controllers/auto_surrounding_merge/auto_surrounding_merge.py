...
from controller import Supervisor, Robot, Node, RadarTarget, Radar, Receiver, Emitter, LED
import array
import struct
import time
import os
...
#        gap = gap2pred - ctrlPara.myMinGap # Previous
        ...
        if gap2pred< ctrlPara.myMaxGap: #Revised        
...
trafficLight = driver.getFromDef("TL_onrmp" + str(ringNum))
...
Veh_Surr_turnGreen = False
Veh_Surr_readtoCount = False
Veh_Surr_Timer = 0

Surr_originPos = [[[ctrlPara.RR1_SurrVeh_Lane1_trans_x,0.51,ctrlPara.RR1_SurrVeh_Lane1_trans_z],[ctrlPara.RR1_SurrVeh_Lane2_trans_x,0.51,ctrlPara.RR1_SurrVeh_Lane2_trans_z],[ctrlPara.RR1_SurrVeh_Lane3_trans_x,0.51,ctrlPara.RR1_SurrVeh_Lane3_trans_z],[ctrlPara.RR1_SurrVeh_Lane4_trans_x,0.51,ctrlPara.RR1_SurrVeh_Lane4_trans_z]],\
                   [[ctrlPara.RR2_SurrVeh_Lane1_trans_x,0.51,ctrlPara.RR2_SurrVeh_Lane1_trans_z],[ctrlPara.RR2_SurrVeh_Lane2_trans_x,0.51,ctrlPara.RR2_SurrVeh_Lane2_trans_z],[ctrlPara.RR2_SurrVeh_Lane3_trans_x,0.51,ctrlPara.RR2_SurrVeh_Lane3_trans_z],[ctrlPara.RR2_SurrVeh_Lane4_trans_x,0.51,ctrlPara.RR2_SurrVeh_Lane4_trans_z]],\
                   [[ctrlPara.RR3_SurrVeh_Lane1_trans_x,0.51, ctrlPara.RR3_SurrVeh_Lane1_trans_z],[ctrlPara.RR3_SurrVeh_Lane2_trans_x,0.51, ctrlPara.RR3_SurrVeh_Lane2_trans_z],[ctrlPara.RR3_SurrVeh_Lane3_trans_x,0.51, ctrlPara.RR3_SurrVeh_Lane3_trans_z],[ctrlPara.RR3_SurrVeh_Lane4_trans_x,0.51, ctrlPara.RR3_SurrVeh_Lane4_trans_z]]]
...
while driver.step() != -1:
    ...
    if not Veh_Surr_turnGreen:
        if trafficLight.getField("state").getSFString() == 'green':
            Veh_Surr_readtoCount = True
            Veh_Surr_turnGreen = True
            #print('Veh_Surr_turnGreen')
        else:
            
            gap2pred=ctrlPara.myOriginGap
            myCurSpeed=0

            avInfo.append( round(driver.getTime(),4))
            avInfo.append( 'RR'+str(ringNum+1))
            avInfo.append( lane_index)
            avInfo.append( 'leaderId')
            avInfo.append('followerId')
            avInfo.append( Veh_name)
            avInfo.append( round(myGps_x,4))
            avInfo.append( round(myGps_z -ctrlPara.gpsZOffset,4))
            avInfo.append( round(myCurSpeed,4))
            avInfo.append( round(gap2pred,4))
            avInfo.append( round(TTC,4))        
            avInfo.append(av_brake_act)
            avInfo.append(BrakeLightOn)
            avInfo.append(av_reaction_pv)
            avInfo.append(cutIn_on)

            
            #global_lock.acquire()
            fcntl.lock(fp, fcntl.LOCK_EX)
            csvwriter.writerow(avInfo)
            #fp.flush()
            #os.fsync(fp)
            fcntl.unlock(fp)
            #global_lock.release()
            ...
            continue
    myGps_x = gps.getValues()[0]
    Which_lane = 1+int((myGps_x - ctrlPara.xoffsetLane[ringNum])/ctrlPara.laneWidth)   #Added

    #Added: Add more waiting for the first lane
    if Which_lane==3: 
        Waiting_time=ctrlPara.DeltT_TL_surr+10
    elif Which_lane==2:
        Waiting_time=ctrlPara.DeltT_TL_surr+10
    else:
        Waiting_time=ctrlPara.DeltT_TL_surr
#    print("Lane Index is",Which_lane,"Waiting_time is",Waiting_time)

            
    if Veh_Surr_readtoCount:
        if Veh_Surr_Timer*time_step/1000<Waiting_time:
            Veh_Surr_Timer += 1
    ...
    if myCurSpeed<200 and myCurSpeed>=0:
        ...
        if isOriginPos:
            ...
            myOriginSeq = int(round((myOriginPos_z - ctrlPara.RR1_SurrVeh_Lane1_trans_z)/ctrlPara.myOriginGap))
        ...
        #print("preVeh", preVeh,"myCurSpeed",myCurSpeed,"gap2pred", gap2pred, "speedDiff",speedDiff,"lane_index", lane_index, "myOriginSeq",myOriginSeq )
        ...
        newSpeed=speed_CACC(preVeh, gap2pred, predSpeed, myCurSpeed, myAccel,CACC_ControlMode, lane_change_action, time_step, ctrlPara.HeadwaySetting[lane_index-1], ctrlPara)  #Revised
        ...
        if speedDiff<0:
            ...
            TTC=TTC_Max    
        # desired speed of leader is constant.
        # if myOriginSeq ==ctrlPara.vehNumber_Lane-1:
            # newSpeed = ctrlPara.myDesSpeed[ringNum]
        ...
        #print("current angle is ",curAngle,"desired angle is ", desAngle,"myGps_x is ", round(myGps_x, 2),"myPosition_Z is ", round(myGps_z, 2))
        ...
        #and myOriginSeq == 0 and lane_index == numLane-1 and ringNum <2:
            ...
            if myCurSpeed <= 0.1:
                ringNum += 1
                myPosition.setSFVec3f([Surr_originPos[ringNum][lane_index-1][0], 0.51, Surr_originPos[ringNum][lane_index-1][2]+myOriginSeq*ctrlPara.myOriginGap])
                trafficLight = driver.getFromDef("TL_onrmp" + str(ringNum))
                tlPosition = trafficLight.getField("translation").getSFVec3f()[2]
