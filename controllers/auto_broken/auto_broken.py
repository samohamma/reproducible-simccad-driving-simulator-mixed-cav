...
#auto_backup_0.auto_backup_0 import global_lock
...
#        gap = gap2pred - ctrlPara.myMinGap # Previous
        ...
        if gap2pred< ctrlPara.myMaxGap: #Revised        
            optSpeed = max(0, min(ctrlPara.myDesSpeed[ringNum],gap/desHeadwayTime))        
        else:
            optSpeed = max(0, min(ctrlPara.myDesSpeed[ringNum],gap/desHeadwayTime))    
        ...
        optSpeed =ctrlPara.myDesSpeed[ringNum]
        print("Broken veh's opt speed is",optSpeed)
...
trafficLight = driver.getFromDef("TL_img_takeOver" + str(ringNum))
...
Veh_Broken_turnGreen = False
Veh_Broken_readtoCount = False
Veh_Broken_Timer = 0

Broken_originPos = [[ctrlPara.RR1_BrokenVeh_trans_x, 0.51, ctrlPara.RR1_BrokenVeh_trans_z],\
                      [ctrlPara.RR2_BrokenVeh_trans_x, 0.51, ctrlPara.RR2_BrokenVeh_trans_z],\
                      [ctrlPara.RR3_BrokenVeh_trans_x, 0.51, ctrlPara.RR3_BrokenVeh_trans_z]]
...
while driver.step() != -1:
    TTC=TTC_Max   
    if not Veh_Broken_turnGreen:
        if trafficLight.getField("state").getSFString() == 'green':
            Veh_Broken_readtoCount = True
            Veh_Broken_turnGreen = True
            print ("broken location is: ", gps.getValues()[2],"tlPosition IS ",tlPosition )
        else:
    ...
    if Veh_Broken_readtoCount:
        if Veh_Broken_Timer*time_step/1000<ctrlPara.DeltT_TL_broken:
            Veh_Broken_Timer += 1
            numStep += 1
            ...
            print ("broken location 1 is: ", gps.getValues()[2])
            Veh_Broken_Timer = 0
            Veh_Broken_readtoCount = False
    ...
    if myCurSpeed<200 and myCurSpeed>=0:
        ...
        #and myOriginSeq == 0 and lane_index == numLane-1 and ringNum <2:
            ...
            if myCurSpeed <= 0.1:
                ringNum += 1
                myPosition.setSFVec3f([Broken_originPos[ringNum][0], 0.51, Broken_originPos[ringNum][2]+myOriginSeq*ctrlPara.myOriginGap])
                trafficLight = driver.getFromDef("TL_img_takeOver" + str(ringNum))
                tlPosition = trafficLight.getField("translation").getSFVec3f()[2]
